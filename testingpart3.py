...
ratings = u.ratings
# Create list of users who have rated that movie
other_ratings = session.query(Rating).filter_by(movie_id=m.movie_id).all()
other_users = []
for r in other_ratings:
    other_users.append(r.user)
# Iterate through user1 ratings and compare to each of user2 ratings to pair up matching movies
o = other_users[0]
paired_ratings = []
for r1 in u.ratings:
    for r2 in o.ratings:
...
# TODO - wait, but isn't this different from method above in terms of going through all the movies? 
